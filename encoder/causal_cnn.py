...
class SqueezeChannels(torch.nn.Module):
    ...
    def forward(self, x):
        # print("x SqueezeChannels", x.size())
        return x.squeeze(2)
...
class CausalConvolutionBlock(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # print("out_causal", out_causal.size())
        res = x if self.upordownsample is None else self.upordownsample(x)
        # print("res", res.size())
        if self.relu is None:
            sum_ = out_causal + res
            # print("out_causal + res", sum_.size())
...
class CausalCNNDecode(torch.nn.Module):
    def __init__(self, in_channels, channels, depth, out_channels,
        ...
        for i in range(depth):
            ...
            dialation_size = dialation_size / 2 # Halves dilation size at each step
            dialation_size = int(dialation_size)
        print("dialation_size:",dialation_size)
        layers += [CausalConvolutionBlock(
...
class CausalCNNEncoder(torch.nn.Module):
    ...
    def forward(self, x):
        # print("Input CausalCnnEncoder", x.size())
        # print("Output CausalCnnEncoder", self.network(x).size())
        return self.network(x)
...
class CausalCNNAutoencoder(torch.nn.Module):
    def __init__(self, in_channels, channels, depth, reduced_size,
        ...
        ## autoencoder
        self.network = torch.nn.Sequential(
            causal_cnn, reduce_size, squeeze, linear, squeeze, reduce_size, causal_cnn_decode
        )
