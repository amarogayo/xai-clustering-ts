...
# from knockknock import email_sender //module for email notification once script is done
import time
...
def fit_hyperparameters(file, train, cuda, gpu, prefix_file, train_labels=None,
                        save_memory=False, data='TRAIN'):
    """
    ...
    return classifier.fit(
        train, train_labels, prefix_file, data, save_memory=save_memory, verbose=True
    )
...
if __name__ == '__main__':
    if whole_dataset:
        ...
        if args.cuda and not torch.cuda.is_available():
            ...
            args.cuda = False
        train, test = load_UEA_dataset(args.path, args.dataset)
        ...
        if not args.load and not args.fit_classifier:
            ...
            classifier = fit_hyperparameters(
                ...
                save_memory=True, prefix_file = os.path.join(args.save_path, args.dataset), data='WHOLE DATASET')
            e = time.time()
        ...
        if not args.load:
            print(os.path.join(args.save_path, args.dataset))
            classifier.save(
                os.path.join(args.save_path, args.dataset)
            )
            with open(
                os.path.join(
                    args.save_path, args.dataset + '_CompleteDATA_hyperparameters.json'
                ), 'w'
            ) as fp:
                json.dump(classifier.get_params(), fp)
            # generate cluster labels
        print("Cluster Whole Dataset")
        # classifier.clusters_complete_dataset(dataset, os.path.join(args.save_path, args.dataset))
    else:
        ...
        if not args.load and not args.fit_classifier:
            s = time.time()
            classifier = fit_hyperparameters(
                args.hyper, train, args.cuda, args.gpu, prefix_file=os.path.join(args.save_path, args.dataset),
                save_memory=True)
            e = time.time()
            print("Processing Time = {}".format(e-s)) 
            processing_time = e-s
        ...
        score = classifier.score(test, os.path.join(args.save_path, args.dataset), test_labels=None)
        score = {'silhouette_score_TEST': score}
        with open(args.save_path + 'score_'+str(processing_time)+'.json', 'w') as f:
            json.dump(score, f)
        print("Test accuracy: " + str(score['silhouette_score_TEST']))

        # classifier.clusters_complete_dataset(train, test, os.path.join(args.save_path, args.dataset))
